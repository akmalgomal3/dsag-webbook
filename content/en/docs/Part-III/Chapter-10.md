...
**Background & Philosophy:**
A standard queue treats all elements equally (<abbr title="First In, First Out queue discipline">FIFO</abbr>). A priority queue reflects the real world: some tasks are simply more important than others and must preempt the queue. The philosophy is "fairness based on weight, not arrival time".
